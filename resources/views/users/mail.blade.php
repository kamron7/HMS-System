...
@section('title', 'Рассылка сотрудникам')
...
<div class="max-w-5xl mx-auto">
    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            <h1 class="text-2xl font-bold text-slate-900 dark:text-slate-100">Рассылка сотрудникам</h1>
            <p class="text-sm text-slate-500 dark:text-slate-400 mt-0.5">Активные сотрудники с email-адресом ({{ $users->count() }})</p>
        </div>
    ...
    <form method="POST" action="{{ route('users.mail.send') }}" id="mail-form">
        @csrf

        <div class="grid grid-cols-1 lg:grid-cols-5 gap-5">

            {{-- Left: user list --}}
            <div class="lg:col-span-2 bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm overflow-hidden flex flex-col" style="max-height:600px">
                ...
                <div class="overflow-y-auto flex-1 divide-y divide-slate-100 dark:divide-slate-700" id="user-list">
                    @forelse($users as $user)
                    <label class="flex items-center gap-3 px-4 py-2.5 hover:bg-slate-50 dark:hover:bg-slate-700/50 cursor-pointer user-row"
                           data-name="{{ strtolower($user->name) }}"
                           data-email="{{ strtolower($user->email) }}">
                        <input type="checkbox" name="user_ids[]" value="{{ $user->id }}"
                               class="user-checkbox rounded border-slate-300 text-blue-600 focus:ring-blue-500 flex-shrink-0">
                        ...
                        <div class="min-w-0">
                            ...
                            <p class="text-xs text-slate-400 truncate">{{ $user->role->label() }} · {{ $user->email }}</p>
                        </div>
                    </label>
                    @empty
                    <div class="px-4 py-10 text-center text-sm text-slate-400">
                        Нет активных сотрудников с email-адресом
                    </div>
                    @endforelse
                </div>
                ...
                <div class="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm p-5">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" id="subject" name="subject" maxlength="200"
                               value="{{ old('subject') }}"
                               placeholder="Напр: Важное объявление для персонала"
                    ...
                    <div class="mb-2">
                        <label for="body" class="block text-xs font-semibold text-slate-600 dark:text-slate-400 mb-1.5">
                            Текст письма <span class="text-red-500">*</span>
                        </label>
                        <textarea id="body" name="body" rows="12" maxlength="5000"
                                  placeholder="Уважаемый сотрудник,&#10;&#10;Хотим сообщить вам..."
                                  class="w-full px-3 py-2.5 border border-slate-200 dark:border-slate-600 rounded-lg text-sm bg-white dark:bg-slate-700 text-slate-900 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-blue-500 resize-none @error('body') border-red-400 @enderror">{{ old('body') }}</textarea>
                        @error('body')<p class="mt-1 text-xs text-red-600">{{ $message }}</p>@enderror
                        <p class="mt-1 text-xs text-slate-400">Имя сотрудника подставляется в приветствии автоматически.</p>
                    </div>
                </div>

                @if($errors->has('user_ids'))
                <p class="text-sm text-red-600">{{ $errors->first('user_ids') }}</p>
                @endif
...
<script>
    ...
    function updateCount() {
        ...
        const visible = [...checkboxes].filter(c => c.closest('.user-row').style.display !== 'none').length;
    ...
    selectAll.addEventListener('change', () => {
        document.querySelectorAll('.user-row').forEach(row => {
            if (row.style.display !== 'none') {
                row.querySelector('.user-checkbox').checked = selectAll.checked;
            }
        });
        updateCount();
    });
    ...
    document.getElementById('mail-form').addEventListener('submit', function (e) {
        ...
        if (n > 5 && !confirm(`Отправить письмо ${n} сотрудникам?`)) e.preventDefault();
    });
