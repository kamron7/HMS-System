...
<div class="mb-6">
    ...
    <h1 class="text-2xl font-bold text-gray-900 mt-2">Редактировать: {{ $user->name }}</h1>
</div>
...
<div class="bg-white rounded-xl border border-gray-200 p-6 max-w-lg">
    <form method="POST" action="{{ route('users.update', $user) }}" class="space-y-5">
        ...
        <div>
            ...
            <p class="mt-1 text-xs text-gray-400">Оставьте пустым, чтобы не менять пароль</p>
            @error('password')
                <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
            @enderror
        </div>
