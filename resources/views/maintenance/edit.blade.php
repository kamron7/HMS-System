...
<div class="max-w-lg mx-auto">
    ...
    <div class="mb-6 flex items-center gap-3">
        ...
        <h1 class="text-xl font-bold text-slate-900">Редактировать</h1>
    </div>
    ...
    <div class="bg-white rounded-xl border border-slate-200 shadow-sm p-6">
        <form method="POST" action="{{ route('maintenance.update', $maintenance) }}" class="space-y-5">
            ...
            <div>
                <label class="block text-sm font-semibold text-slate-700 mb-1.5">Номер</label>
            ...
            <div>
                ...
                <input type="text" id="title" name="title" value="{{ old('title', $maintenance->title) }}" required maxlength="150"
                       class="w-full px-3 py-2.5 border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 {{ $errors->has('title') ? 'border-red-400' : 'border-slate-200' }}">
                @error('title')<p class="mt-1 text-xs text-red-600">{{ $message }}</p>@enderror
            </div>

            <div>
                <label for="description" class="block text-sm font-semibold text-slate-700 mb-1.5">Подробности</label>
                <textarea id="description" name="description" rows="3" maxlength="2000"
                          class="w-full px-3 py-2.5 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 resize-none">{{ old('description', $maintenance->description) }}</textarea>
            </div>
            ...
            <div>
                ...
                <select id="priority" name="priority" required
                    ...
                    @foreach($priorities as $p)
                        <option value="{{ $p->value }}" {{ old('priority', $maintenance->priority->value) === $p->value ? 'selected' : '' }}>
                            {{ $p->label() }}
                        </option>
                    @endforeach
                </select>
            ...
            <div>
                <label for="assigned_to" class="block text-sm font-semibold text-slate-700 mb-1.5">Назначить сотрудника</label>
                <select id="assigned_to" name="assigned_to"
                        class="w-full px-3 py-2.5 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                    <option value="">— Не назначен —</option>
                    @foreach($staff as $member)
                        <option value="{{ $member->id }}" {{ old('assigned_to', $maintenance->assigned_to) == $member->id ? 'selected' : '' }}>
                            {{ $member->name }}
                        </option>
                    @endforeach
                </select>
